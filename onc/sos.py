...
class SOS:
    ...
    def getCapabilities_SOAP(self):
        ...
        if response.ok:
            ...
            operation = dict["{0}Capabilities".format(sos)][
                "{0}OperationsMetadata".format(ows)
    ...
    def describSensor_SOAP(self, sensor):
        ...
        if response.ok:
            ...
            member = dict["{0}SensorML".format(sensorML)]["{0}member".format(sensorML)][
                "{0}System".format(sensorML)
            ]
            ...
            if "{0}input".format(sensorML) in inputs:
                for i in inputs["{0}input".format(sensorML)]:
                    name = i["@name"]
                    sensorDef = i["{0}ObservableProperty".format(swe)][
                        "@definition"
                    ].split(":")
    ...
    def getObservation_SOAP(
        self, offering, begin, end, observedProperty, propertyDictionary={}
    ):
        result = {}
        url = "{}sos?WSDL".format(self.baseUrl)
        headers = {"content-type": "application/soap+xml"}
        ...
        observedProperties = "\n".join(
            [
                "                    <observedProperty>urn:ogc:def:phenomenon:{}</observedProperty>".format(
                    p
                )
                for p in observedProperty
            ]
        )
        ...
        body = """<?xml version="1.0" encoding="UTF-8"?>
                ...
                </GetObservation>""".format(
            offering, begin, end, observedProperties
        )
        ...
        if response.ok:
            ...
            try:
                if (
                    member["@{http://www.w3.org/1999/xlink}href"]
